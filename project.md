...
- Feel familiar: the interactions people already know from any video site, with our own fonts and colors — and a Light / Dark / Auto choice in the top bar. The choice rides in a cookie, so the server renders `data-theme` on `<html>` itself and the first paint is already right: no inline script, no flash, no hydration mismatch. Auto is the absence of a cookie, so it keeps following the device — a deep forest green with warm cream surfaces, Source Serif 4 for display type and Figtree for everything else. Colours are tokens in `globals.css`; no component names a colour.
...
- **Observability**: one log line per request with a request id (returned as `x-request-id` and carried on every line of that request), the signed-in user, status and duration. `LOG_LEVEL` and `LOG_FORMAT` in `.env`; `json` emits one object per line for Cloud Logging. Decisions that would otherwise be invisible are logged with their reason: why a 401 happened, which login check failed (the response stays identical), uploads starting and finishing, AI provider/model/latency, refresh-token reuse. At `LOG_LEVEL=DEBUG` a call reads as a trace: an entry line, every SQL statement with its duration, each storage and AI round-trip, each health check timed, what the handler decided, and the response before and after transfer. Queries over 500ms are logged at INFO regardless. Still to come: OpenTelemetry traces → Cloud Trace, error alerts.
